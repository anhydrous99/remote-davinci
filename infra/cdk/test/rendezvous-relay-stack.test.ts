...
describe('RendezvousRelayStack', () => {
  it('contains only the requested serverless boundary', () => {
    ...
    stack.resourceCountIs('AWS::CloudWatch::Alarm', 4);
  ...
  it('enables reduced production access logging only when requested', () => {
    const stack = template('prod', true);
    stack.resourceCountIs('AWS::Logs::LogGroup', 2);
    const logs = stack.findResources('AWS::Logs::LogGroup');
    const accessLog = Object.entries(logs).find(([id]) => id.startsWith('AccessLogs'));
    assert.ok(accessLog);
    ...
    const rendered = JSON.stringify(accessLogSettings);
    assert.match(rendered, /requestId/);
    assert.match(rendered, /routeKey/);
    assert.doesNotMatch(rendered, /connectionId/);
    assert.doesNotMatch(rendered, /apiId/);
  });
